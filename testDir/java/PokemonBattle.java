...
import engine.BattleServer.Interrupt;
import engine.pokemon.*;
import engine.pokemon.effects.*;
import engine.pokemon.effects.BattleEffectCollection;
import engine.pokemon.moves.DamageInformation;
import engine.pokemon.moves.Move;
import engine.pokemon.moves.Schedule;
import engine.pokemon.moves.Schedule.ScheduleItem;
import etc.MyObject;
...
public class PokemonBattle extends MyObject implements Iterable<Pokemon> {
	...
	public PokemonBattle(BattleRule rule) {
		this.rule = rule;
		this._players = new etc.BufferList<>(rule.NumberPlayers);
	...
	public void execute() {
		...
		/*
		 * Move mv = null; while (_run) { Prompt.prompt(this.toString());
		 * executeSchedule(); //if(canAttack(_player.getCurrPokemon()))continue; TODO
		 * int input = Prompt.prompt("Please select an option", "Fight", "Bag",
		 * "Pokemon", "Exit"); if (input == 0) {
		 * 
		 * while (mv == null) { mv = playerChoose(); }
		 * 
		 * // if(_run)break; mv.init(_schedule, PLAYER1, PLAYER2);
		 * 
		 * mv = enemyChoose(); mv.init(_schedule, PLAYER2, PLAYER1);
		 * 
		 * _schedule.incrementRound(); } else if (input == 2) {
		 * chooseNewPlayerPokemon(); } else if (input == 3) { _run = false; exit(); } mv
		 * = null;
		 * 
		 * }
		 */
	...
	protected BattleRoundResult executeSchedule(Interrupt interrupt) {
	 ...
	 */
	private etc.BufferList<Player> _players;

	public etc.BufferList<Player> getPlayers() {
