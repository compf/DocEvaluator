...
Durch diesen Tupel kann für jedes Teilergebnis die Gewichtung der Metrik, der Komponente und des Dateipfades  abgerufen werden. Durch Multiplikation  aller Gewichtungen entsteht ein Gesamtgewicht. Entscheidet sich ein Benutzer beispielsweise dafür, dass die Metrik \enquote{Anteil dokumentierter öffentlicher Komponenten an allen öffentlichen Komponenten} (Kapitel \ref{chapter:public_members}) doppelt zu gewichten und die Datei \enquote{Main.java} dreifach zu gewichten (ansonsten aber keine Komponenten besonders zu gewichten), so würde jedes Einzelergebnis, das aus der Analyse dieser Datei mit der besagten Metrik stammt, mit den Faktor $2*3*1=6$ gewichtet werden. Alle anderen Dateien, die aber ebenfalls von dieser Metrik analysiert werden, erhalten die Gewichtung $2*1*1=2$.  Teilergebnisse von anderen Metriken, die ebenfalls die \enquote{Main.java} bewerten, würden entsprechend mit dem Faktor $1*3*1=3$ gewichtet werden.
