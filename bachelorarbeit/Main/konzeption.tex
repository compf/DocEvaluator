...
Die Baumstruktur aus dem Parsing-Prozess wird nun von dem Metriken genutzt, um die Dokumentationsqualität zu ermitteln. Jede zu implementierende Metrik muss durch eine separate Klasse, welche von der Klasse \textit{DocumentationAnalysisMetric} erbt, in das Tool eingebunden werden. Die standardmäßig implementierten Metriken werden später in Kapitel \ref{chapter:metrics} genauer erläutert. putAllLogMessages
Dabei können zwei verschiedene Kategorien unterschieden werden. Die Metriken der ersten Kategorie arbeiten nur auf eine Komponente und der dazugehörigen Dokumentation. Andere Metriken sind nur mit hierarchischen Komponenten kompatibel und analysieren die Kommentare all ihrer Kinder. Dies ist beispielsweise für die Metrik in Kapitel \ref{chapter:metrics_loc_ratio} sinnvoll.

Eine Klasse, die eine Metrik darstellt, muss mindestens die Methoden \textit{shallConsider} und \textit{analyze} implementieren. Durch die Methode \textit{shallConsider}, welche eine Komponente als Parameter erhält, kann der Aufrufer der Metrik erfahren, ob diese Komponente überhaupt interessant für diese Metrik ist. Eine Metrik, die nur die Dokumentation einer Methode analysieren kann, muss durch diese Überprüfung nicht die Dokumentation einer Klasse bewerten. In der Methode \textit{analyze} geschieht der eigentliche Analysevorgang. Diese Methode erhält ebenfalls die zu analysierende Komponente und einen Verweis auf einen \textit{MetricResultBuilder}, mit dem die Metrik ihre Ergebnisse veröffentlichen kann. Daher gibt die Methode \textit{analyze} keinen Wert zurück, sondern nutzt die Methode \textit{processResult} des \textit{MetricResultBuilder}. Dies hat den Vorteil, dass nicht zwingend ein Ergebnis bzw. ein ungültiges Ergebnis zurückgegeben werden muss, wenn beispielsweise eine Komponente trotz eines positiven \textit{shallConsider} doch nicht analysiert werden sollte. 

Eine Metrik benötigt unter Umständen auch weitere Informationen, um bestimmte Entscheidungen zu treffen. Dazu erhält die Metrik ein Parameterobjekt. Die Struktur der Parameter hängt stark von der Metrik ab, sodass es hier keine einheitliche Schnittstelle gibt. Stattdessen wird der Typ \textit{any} von TypeScript/JavaScript verwendet. 
Da es von einer Metrikklasse mehrere Instanzen geben kann, müssen diese eindeutig unterschieden werden. Dies wird hier mittels eines eindeutigen Namens gelöst, der vom Benutzer der Metrik vergeben wird. Durch die Verwendung von mehreren Instanzen können beispielsweise unterschiedliche Parametrisierung der gleichen Metrik ausgeführt werden.

Wenn die Metrik das Resultat einer Analyse veröffentlichen will, benötigt sie die numerische Bewertung, eventuelle Nachrichten, die der Benutzer des Tools als Feedback nutzen kann und eine Zeichenkette, die zur Zuordnung eines Gewichtes relevant ist. 
...
Die Nachrichten helfen den Benutzer dabei, Komponenten mit schlechter Dokumentation zu finden und die Qualität der Kommentare zu verbessern. Dazu wird pro Nachricht der Name der betroffenen Komponenten, die Zeilennummer und der Dateiname genannt. Anschließend folgt ein Text, den die implementierte Metrik frei befüllen kann. 
